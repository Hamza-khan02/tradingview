 ...
 */
async function openAlertDialog(cdpClient: CdpClient): Promise<void> {
...
export async function createAlert(
  cdpClient: CdpClient,
  params: { price: number; name?: string; message?: string }
  ...
  const result = await cdpClient.evaluate<{ success: boolean; message: string }>(`
    (() => {
...
export async function listAlerts(cdpClient: CdpClient): Promise<AlertItem[]> {
  ...
  // Scrape list items
  return cdpClient.evaluate<AlertItem[]>(`
    (() => {
      const list = [];
...
export async function deleteAlert(cdpClient: CdpClient, index: number): Promise<{ success: boolean; message: string }> {
