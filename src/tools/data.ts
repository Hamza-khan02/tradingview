...
export async function getCurrentSymbol(cdpClient: CdpClient): Promise<SymbolDetails> {
  logger.info("Extracting current symbol...");
  return cdpClient.evaluate<SymbolDetails>(`
    (() => {
      let symbol = "";
      let exchange = null;
...
export async function getCurrentTimeframe(
  cdpClient: CdpClient
): Promise<any> {
...
export async function getIndicatorValues(cdpClient: CdpClient): Promise<IndicatorValue[]> {
  logger.info("Scraping indicator values...");
  return cdpClient.evaluate<IndicatorValue[]>(`
    (() => {
      const results = [];
...
export async function getOhlcvData(cdpClient: CdpClient): Promise<OhlcvBar[]> {
  logger.info("Fetching OHLCV data...");
  return cdpClient.evaluate<OhlcvBar[]>(`
    (() => {
