...
import { changeSymbol, changeTimeframe, refreshChart, getChartScreenshot, getChartLayout, } from "./tools/chart.js";
import { getCurrentSymbol, getCurrentTimeframe, getIndicatorValues, getOhlcvData, } from "./tools/data.js";
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const ensureConnected = () => {
        if (!cdpClient.isConnected()) {
            throw new Error("TradingView Desktop remote debugging is disconnected. " +
...
runServer().catch((err) => {
    logger.error(`Fatal crash in runServer: ${err.message}`);
    process.exit(1);
});
